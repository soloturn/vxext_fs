...
static ssize_t fat_direct_IO(struct kiocb *iocb,
...
static int fat_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	#ifndef VXEXT_FS
	buf->f_namelen = (sbi->options.isvfat ? FAT_LFN_LEN : 12) * NLS_MAX_CHARSET_SIZE;
	#else
		   ...
		   void (*setup)(struct super_block *))
{
   ...
   printk(KERN_INFO "sec_per_clus2.....: %d\n",   le16_to_cpu(get_unaligned((__le16 *)&b->fat32.sec_per_clus2)));
   ...
   {
      if(!silent)
         fat_msg(sb, KERN_ERR, "VXEXT: bogus total_sectors");
   ...
   if(b->sec_per_clus == 0)
      sbi->sec_per_clus = b->fat32.sec_per_clus2;
   else
   ...
   {
     if(!silent)
       fat_msg(sb, KERN_ERR, "VXEXT: bogus sectors per cluster %u",
              sbi->sec_per_clus);
	...
	if (!is_power_of_2(sbi->sec_per_clus)) {
		if (!silent)
			fat_msg(sb, KERN_ERR, "FAT: bogus sectors per cluster %u",
			       sbi->sec_per_clus);
		brelse(bh);
		goto out_invalid;
	}
   ...
   fat_msg(sb, KERN_INFO, "VFS: successfully mounted VXEXT1.0 filesystem from device %s.", sb->s_id);
   #endif
